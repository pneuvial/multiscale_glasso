...
# Applications 
<revision> To illustrate the proposed simultaneous graph and clusters inference approach, we applied MGLasso to a microbial association data to study the multiscale structures between operational taxonomic units, as well as methylation and transcriptomic samples extracted from the Evolutionary and functional impact of EPIgenetic variation in forest TREEs project [@maury2019epigenetics]</revision>
...
grouped in a dataset composed of $n_1 = 289$ for $127$ OTUs. <revision> The filtered dataset is available in the SpiecEasi R package available on Github. </revision> 
<old>
...
clustering structure. <old> The graphs obtained by MGLasso for $\lambda_2 = 0$
and for $\lambda_2 = 5$ (corresponding respectively $127$ and $80$
clusters) are given below. In each case,</old> The parameter $\lambda_1$ is
chosen by stability selection. <revision> The hyperparameter of the StARS approach is set to $0.05$ which might select the Lasso penalty that leads to a sparse and stable graph between subsampled graphs. <revision> The grid of values taken by the fusion parameter $\lambda_2$ is set to $[0, 20].$ The generated clustering path of solutions for parameter $\lambda_2$ is given in @fig-clusterpath We project the data and the predicted $\hat{\boldsymbol X}$ computed from the estimated multiple regression vectors at each $\lambda_2$ on the two principal components of the data. Note that the path is not always agglomerative. An extensive study should be carried on finding the weights for which MGLasso can be guranteed to produce agglomerative paths. However these splits between clusters ensure an optimal solution.
...
plot_clusterpath(amgut1.filt, mgl_amgut_rev)
```

Clustering path of the MGLasso convex clustering solutions using $127$ OTUS by plotting the original data and predicted data onto the two principal components of original data.
:::
...
The fusion threshold has been set to $0.001.$
<!-- ::: {#fig-real-data-mglasso} -->
<!-- ```{r} -->
...
Comparison of results with taxanomic classes of Rank3
...
clusters <- as.character(mgl_amgut_rev$`1`$clusters)
taxas <- amgut1.filt.phy@tax_table@.Data
taxas <- cbind(clusters, taxas)

plot_network(ig.mgl.tv0, taxas, type = "taxa", layout.method = layout_with_fr, color = "Rank3")


ig.mgl.tv1 <- adj2igraph(mgl_amgut_rev$`20`$selected_Theta)
E(ig.mgl.tv1)$weight <- abs(E(ig.mgl.tv1)$weight) 

#get taxa table and add add new column for clusters
clusters <- as.character(mgl_amgut_rev$`20`$clusters)
taxas <- amgut1.filt.phy@tax_table@.Data
taxas <- cbind(clusters, taxas)
...
The rand index between the MGLasso partition in ... clusters and the Rank 3 taxonomic classification is `r mclust::adjustedRandIndex(taxas[,"clusters"], taxas[,"Rank3"])`
