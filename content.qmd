...
<!-- \begin{equation} -->
<!--    (\boldsymbol \beta^i,  \textcolor{blue}{\boldsymbol \tau_{ij}} \boldsymbol \beta^j)  =  -->
<!--    \begin{pmatrix} -->
<!--         \beta_1^i & \beta_2^i & \dots & \tikzmarknode{a}{\beta_j^i} & \dots \dots & \tikzmarknode{b}{\beta_k^i} & \dots & \beta_p^i\\ -->
<!--         \\ -->
<!--         \\ -->
<!--         \beta_1^j & \beta_2^j & \dots & \tikzmarknode{c}{\beta_k^j} & \dots \dots & \tikzmarknode{d}{\beta_i^j} & \dots & \beta_p^j -->
<!--     \end{pmatrix} -->
<!-- \end{equation} -->
...
tryCatch(conesta_rwrapper(1), finally = "Test result")
```
...
save(runtime_rand50_config_p40_bdiagflip001_allcor, 
     file = paste0(path_data, "runtime_rand100_config_p40_bdiagflip001_allcor.RData"))
save(rand50_config_p40_bdiagflip001_allcor, 
     file = paste0(path_data, "rand100_config_p40_bdiagflip001_allcor.RData"))
...
save(runtime_rand100_config_p40_bdiagflip001_cor95, 
     file = paste0(path_data, "runtime_rand100_config_p40_bdiagflip001_allcor.RData"))
save(rand50_config_p40_bdiagflip001_allcor, 
     file = paste0(path_data, "rand100_config_p40_bdiagflip001_allcor.RData"))
```
...
## Rand index  
load(paste0(path_data, "rand100_config_p40_bdiagflip001_allcor.RData")) 
...
save(dt_rand,
     file = paste0(path_data, "dt_rand_p40_bdiagflip001_allcor_thresh_e6.RData"))
...
The collected DNA methylation data are proportions, and expression data are
counts data. Details on the plant material and experimental design can be found
in @sow2019role and @chateigner2020gene. The transcriptomic data were measured
via RNA-Seq and normalized using Trimmed Mean of M-Values combined with a Best
linear unbiased predictor (BLUP) correction as described in @chateigner2020gene.
The methylation data were measured through whole-genome bisulfite sequencing
(WGBS) and are normalized via the classical logarithmic approach $log_2(x+1)$
where $x$ is the methylation proportion. For each one of the $10$ populations
(see @fig-context-epitree), DNA methylation in CG, CHG, and CHH contexts for
promoters and gene-body and RNA sequencing data are observed on genotypes. A
mean measure is computed from two replicates per population. The analysis has
been restricted to a set of $150$ target genes which explains the most
variability in the omics data and the subsequent number of samples from
