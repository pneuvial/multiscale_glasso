...
MGLasso exploits the multitask learning framework for GGMs inference coupled with a convex clustering problem over the nodes to simultaneously infer multiscale networks and clusters. To the best of our knowledge, this is the first attempt in the litterature of undirected GGMs. Namely we use the novel fusion term $\sum_{i < j} \left \lVert \boldsymbol{\beta}^i - \tau_{ij}(\boldsymbol{\beta}^j) \right \rVert_2$ coupled with a sparsity penalty on the vectors. The function $\tau_{ij}$ is a permutation on symmetric coefficients. MGLasso can also be seen as a sort of sparse group fused Lasso for graphical models and can be straightforwardly extended to probability distributions belonging to the exponential family [@yang2012graphical]. 
...
$\|\boldsymbol{\beta}^i - \tau_{ij} (\boldsymbol{\beta}^j)\|_2=0$.
...
$\boldsymbol D_{ij} \tilde{\boldsymbol{\beta}} = \boldsymbol{\beta}^i - \tau_{ij}(\boldsymbol{\beta}^j).$
</edmond>
