...
proposed by @Liu2010 <revision> as suggested by some preliminary tests where we compared the EBIC, the BIC criterion calibrated with slope heuristics \citep{baudry2012slope}, the Rotation Invariant Criterion implemented in the Huge package \citep{zhao2012huge}, the GGMSelect procedure \citep{giraud2012graph}, cross-validation and STARS with very stable graphs (hyperparameter set to $0.1$) versus STARS with completely unstable graphs (hyperparameter set to $0.5$) between subsamples.</revision> . The method uses $k$ subsamples of data to estimate
