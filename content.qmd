...
$\boldsymbol A_{ij}$ is a $p\times p(p-1)$ matrix ~~defined by~~  <edmond> chosen so that $\boldsymbol D_{ij} \tilde{\boldsymbol{\beta}} = \boldsymbol{\beta}^i - \tau_{ij}(\boldsymbol{\beta}^j).$ Some practical details about the implementation are provided in appendix. </edmond>
